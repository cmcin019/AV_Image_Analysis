...
class BaseSegmentor(BaseModule, metaclass=ABCMeta):
    ...
    def show_result(self,
        ...
        for label, color in enumerate(palette):
            ...
            if label in [12, 13, 14, 15, 17, 18]:
                color_seg[seg == label, :] = [255,255,255]
            else:
                #color_seg[seg == label, :] = color
                color_seg[seg == label, :] = [0,0,0]
        # convert to BGR
        ...
        kernel[k_dim//2:k_dim] = 0
        img = mmcv.adjust_sharpness(img, factor=0.0, kernel=kernel)
