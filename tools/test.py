...
def to_vid():
    dir_list = os.listdir('road_depth/')

    dir_list.sort()
    
    img = cv2.imread('road_depth/'+dir_list[0])
    height, width = img.shape[:-1]
    
    out = cv2.VideoWriter('videos/depth_001.avi',cv2.VideoWriter_fourcc(*'DIVX'), 8, (width, height))
    
    for x in dir_list:
    	if 'png' in x:
	    	img = cv2.imread('road_depth/'+x)
	    	out.write(img)
    out.release()
